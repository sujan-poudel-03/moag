...
export function activate(context: vscode.ExtensionContext): void {
  ...
  // Self-reporting reads live counters, so it takes getters rather than values.
  // Getters, not values: currentPlan and the sidebar provider are reassigned
  // during a session, and a snapshot taken here would go stale immediately.
  initPrdAuthoring({
    get currentPlan() { return currentPlan; },
    get currentPlanPath() { return currentPlanPath; },
    get extensionContext() { return extensionContext; },
    get promptViewProvider() { return promptViewProvider; },
    get runner() { return runner; },
    detectPrdFileInWorkspace,
    getActivePlan,
    injectTestTaskIfMissing,
    saveAndRefresh,
    syncPromptProviderSidebar,
  });
  ...
  initSelfReporting({
    ...
    executionTaskCount: () => executionTaskCount,
    executionTasksFailed: () => executionTasksFailed,
    appendOutput: (line) => moagOutputChannel?.appendLine(line),
  });
...
async function createPRFromRun(): Promise<void> {
  ...
  if (!isGhAvailable(cwd)) {
    vscode.window.showWarningMessage(
      describeFailure({ ok: false, reason: 'gh-missing', message: '' }),
    );
    return;
  }
  ...
  const title = await vscode.window.showInputBox({
    prompt: 'PR title',
    value: plan?.name || 'MOAG automated changes',
  });
  if (!title) { return; }
  ...
  const outcome = createPullRequest({
    ...
    title,
    planName: plan?.name ?? '',
    tasks: runEntries.map((e) => ({ name: e.taskName, completed: e.status === 'completed' })),
    changedFiles: [...new Set(runEntries.flatMap((e) => e.changedFiles ?? []))],
  });

  if (!outcome.ok) {
    const text = describeFailure(outcome);
    if (outcome.reason === 'failed') { vscode.window.showErrorMessage(text); }
    else { vscode.window.showWarningMessage(text); }
    return;
  }

  const action = await vscode.window.showInformationMessage(`PR created: ${outcome.url}`, 'Open PR');
  if (action === 'Open PR' && outcome.url) {
    vscode.env.openExternal(vscode.Uri.parse(outcome.url));
  }
}
...
// .moag/ path resolution lives in workspace/moag-paths.ts
import {
  getMoagDir,
  getMoagPlanPath,
  ensureMoagDir,
...
// Self-reporting (friction, error log, report panel) lives in ui/self-reporting.ts
import {
  initSelfReporting,
  collectRunFriction,
  appendFrictionLog,
  appendErrorLog,
  cmdRunContextEval,
  cmdAnalyzeContextWeights,
  classifyMoagIssue,
  logMoagSelfIssue,
  cmdReviewSelfIssues,
  cmdShareRunFeedback,
  cmdReportIssue,
} from './ui/self-reporting';
...
// PRD authoring (panel, chat, AI improve, versions) lives in ui/prd-authoring.ts
import {
  initPrdAuthoring,
  handlePrdChatAnswer,
  cmdSavePrdToPlan,
  ...
  handlePrdAiImprove,
  openPrdInBrowser,
  savePrdVersionToActivePlan,
  cmdPlanFromPRD,
  generatePlanFromPrdText,
} from './ui/prd-authoring';
